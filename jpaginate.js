// jPaginate Plugin for jQuery - Version 0.4
// Updated by Method-Development
/* Fixed Issues:
   -When using the "Minimize" and "Nav_items" options the results would not appear correctly or would show more than intended. This has been fixed
   Added Options:
   -Added first page and last page button
*/
    ...
    $.fn.jPaginate = function (options) {
        ...
        return this.each(function () {
            ...
            function createPagination(curr) {
                ...
                //var firstPage = "<li><a class='goto_previous' href='#'>" + options.previous + "</a></li>";
                var firstPage = "<li><a class='goto_first' href='#'>" + options.firstPage + "</a></li>";
                var lastPage = "<li><a class='goto_last' href='#'>" + options.lastPage + "</a></li>";
                var previous = "<li><a class='goto_previous' href='#'>" + options.previous + "</a></li>";
                var next = "<li><a class='goto_next' href='#'>" + options.next + "</a></li>";
                var firstPage_inactive = "<li><a class='inactive' href='#'>" + options.firstPage + "</a></li>";
                var lastPage_inactive = "<li><a class='inactive' href='#'>" + options.lastPage + "</a></li>";
                ...
                if (curr != 1 && curr != number_of_pages) {
                    nav = start + firstPage + previous + items + next + lastPage + end;
                } else if (number_of_pages == 1) {
                    nav = start + firstPage_inactive + previous_inactive + items + next_inactive + lastPage_inactive + end;
                } else if (curr == number_of_pages) {
                    nav = start + firstPage + previous + items + next_inactive + lastPage_inactive + end;
                } else if (curr == 1) {
                    nav = start + firstPage_inactive + previous_inactive + items + next + lastPage + end;
                }
...
((expiredays == null) ? "" : ";expires=" + exdate.toUTCString());
            ...
            function paginationCalculator(curr) {
                ...
                var upper_limit = number_of_pages - options.nav_items + 1;
                ...
                //var start = parseInt(curr) > parseInt(half) ? Math.max(Math.min(curr - half, upper_limit), 0) : 0;
                var end = parseInt(curr) > parseInt(half) ? Math.min((parseInt(curr) + parseInt(half) + parseInt(options.nav_items) % 2) - 1, parseInt(number_of_pages)) : Math.min(options.nav_items, number_of_pages);

                //var test = Math.min((parseInt(curr) + parseInt(half) + options.nav_items % 2), parseInt(number_of_pages));
                //
            ...
            $(".goto_first").live("click", function (e) {
                e.preventDefault();
                var act = "." + options.active;
                //var newcurr = parseInt($(".pagination").find(".active").attr("title")) - 1;
                //set_cookie("current", newcurr);
            ...
            $(".goto_last").live("click", function (e) {
                e.preventDefault();
                var act = "." + options.active; 
                //var newcurr = parseInt($(".pagination").find(".active").attr("title")) - 1;
                //set_cookie("current", newcurr);
                showPage(number_of_pages);
                $(".pagination").remove();
